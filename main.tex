...
\vspace{15pt}
Задача данной работы состоит в решении нелинейного (трансцендентного) уравнения \eqref{eq:trans_eq}, где $F(x)$ определена формулой \eqref{eq:trans_func}.
...
методу Стеффенсена \cite{Press,Steffensen}. (При $\delta x_k \to 0$ рис. 4 также иллюстрирует работу метода  MSe).
...
Метод обладает квадратичной скоростью сходимости (p = 2)~\cite{Press}.
...
Решение уравнения $F(x)=0$, где F(x) задана формулой \eqref{eq:trans_func_specific}, выполнено численно на интервале $(0, \frac{3\pi}{2})$ с точностью $10^{-12}$, которая обеспечивается критериями остановки, принятыми для каждого метода (невязка и/или длина интервала).
Вычисления выполнены в среде \textsc{Julia} и \textsc{Matlab}.
Ниже приведен график функции \eqref{eq:trans_func_specific} на интервале~$(0, \frac{3\pi}{2})$.
...
\vspace{4pt}
Ниже приведем код, выполненный в среде \textsc{Julia}, и блок-схему метода MSe с подробным разбором. Реализация остальных методов приведена в приложении (\texttt{numMethods.jl}).
...
function modified_secant(F::Function, x0::Float64; tol, maxiter)
    ...
    while it < maxiter 
        ...
        @assert denom != 0.0 "(*@\text{Деление на ноль!}@*)" # (*@\text{Проверка деления на ноль}@*)
        xnew = x - d * fx / denom # (*@\text{Вычисление нового приближения}@*)
        # (*@\text{Проверка критерия остановки по приращению аргумента}@*)
        if abs(xnew - x) < tol
            return xnew, it + 1 # (*@\text{Возвращаем результат}@*)
        end
...
успешно сходится. Искомое значение достигнуто на \text{23-й}
итерации и составляет \textbf{$x = 1.570799848029253$}. \\

Для более детального анализа процесса поиска корня был построен график 
сходимости метода (рис.~8), отражающий 
зависимость абсолютной невязки $|F(x_k)|$ от номера итерации $k$ 
...
\begin{minipage}{0.9\textwidth}
    \centering
    \includegraphics[width=\linewidth]{residual_mse.png}
    \caption{График сходимости метода MSe для $x_0=1.5$}
\end{minipage}
\end{figure}


Из графика видно, что на первых пяти итерациях метод демонстрирует 
высокую скорость сходимости, стремительно снижая невязку 
с $10^{-2}$ до значений порядка $10^{-9}$. Далее, в диапазоне с 5-й 
по 22-ю итерацию, наблюдается плато: значение $|F(x_k)|$ осциллирует 
в пределах от $10^{-9}$ до $10^{-11}$. На 23-й итерации алгоритм успешно завершает работу, 
удовлетворив критериям останова.
...
\midrule
Bisection         & 1.570799848595700 & 42 & $3.52180080\cdot10^{-6}$ \\
Trisection        & 1.570799848553825 & 27 & $3.52175893\cdot10^{-6}$ \\
False Position    & 1.530143432659914 & 100 & $4.06528941\cdot10^{-2}$ \\
Newton--Raphson   & 1.570799848892807 & 43  & $3.52209791\cdot10^{-6}$ \\
Secant            & 1.570799848657968 & 30  & $3.52186307\cdot10^{-6}$ \\
Modified Secant   & 1.570799848029253 & 23 & $3.52123436\cdot10^{-6}$ \\
FP--MSe           & 1.570799848646989 & 100  & $3.52185209\cdot10^{-6}$ \\
Halley            & 1.570799848686475 & 22  & $3.52189158\cdot10^{-6}$ \\
Ridders           & 1.570799848862333 & 11  & $3.52206744\cdot10^{-6}$ \\
VW--Brent         & 1.570799848663367 & 22 & $3.52186847\cdot10^{-6}$ \\
Steffensen        & 1.570799848029253 & 23  & $3.52123436\cdot10^{-6}$ \\
MSt               & 1.570799848711005 & 4  & $3.52191611\cdot10^{-6}$ \\
fzero (\textsc{Matlab})  & 1.570799848445529 & --  & $3.52165063\cdot10^{-6}$ \\
\midrule
...
начального приближения ($x_0 = 1.5$).
В этой ситуации метод MSt
демонстрирует самую быструю сходимость.
...
Для методов Newton, Halley, MSe, MSt и метода
...
\begin{minipage}{1.01\textwidth}
    ...
    \small Рис. 9 Карта сходимости методов
\end{minipage}
\end{figure}


Для количественной характеристики эффективности методов была
построена диаграмма числа выигранных точек (рис.~10).
...
Рисунки 9 и 10 были построены при помощи библиотеки \texttt{matplotlib} в \texttt{Python} \cite{matplotlib}.

\newpage

Таким образом, результаты, полученные на сетке начальных данных,
согласуются с выводами, сделанными на основе частного примера,
приведённого в таблице~\ref{tab:tau_compare}.
Наибольшее число выигранных точек демонстрирует метод Modified Steffensen (MSt),
который существенно превосходит остальные методы.
Mетоды Ridders и NR
занимают второе и третье места. Методы Secant и Halley занимают четвертое и пятое места. При текущей стратегии выбора приращения аргумента
($\delta x_k = F(x)$) метод MSe занимает шестое место.
Следовательно, наиболее устойчивыми к выбору начальных приближений
оказались методы MSt, Ridders и NR.
