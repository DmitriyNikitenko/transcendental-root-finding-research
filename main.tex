...
Также данный метод сохраняет интервал локализации корня.
\end{minipage}
\hspace{7pt}
\hfill
...
начального приближения (точка A на рис. 4).
\end{minipage}
\hfill
...
\begin{itemize}
    ...
    \item \textbf{Метод дихотомии}: выполняется, если новое приближение $b + d$ выходит за границы интервала локализации $(b, c)$ или если вычисленный интерполяционный шаг $d$ убывает медленнее шага дихотомии.
\end{itemize}
...
Решение уравнения \eqref{eq:trans_eq} выполнено численно на интервале $(0, \frac{3\pi}{2})$ с точностью $10^{-12}$, которая обеспечивается критериями остановки, принятыми для каждого метода (невязка и/или длина интервала).
